...
//
//int nn, max, grade;	//nn,全局变量，用来取blocks[base][space_z]中的base，表示7种基础图形之一
//int main()
//{
//	max = 0, grade = 0;
...
//	srand((unsigned int)time(NULL));	//main函数中设置srand((unsigned)time(NULL));时，影响的将是所有的rand
...
////	}
////	fseek(fp, 0, 0);	//函数设置文件指针stream的位置。如果执行成功，stream将指向以fromwhere
...
//				for (j = 0; j < 4; j++)
//				{
//					blocks[base][space_z + 1].space[i][j] = tem[4 - j - 1][i];	
//					//控制一边坐标不变，另一边为4 - i - 1，然后再行列互换，这样可以保证四次旋转不同，
...
//	HANDLE hOut = GetStdHandle(STD_OUTPUT_HANDLE);	//HANDLE句柄指的是一个核心对象在某一个进程中的唯一索引
//	//不是指针。GetStdHandle是一个Windows API函数。它用于从一个特定的标准设备（标准输入、标准输出或标准错误）
//	//中取得一个句柄（用来标识不同设备的数值）。
...
//	//SetConsoleTextAttribute是一个API（应用程序编程接口），可以设置控制台窗口字体颜色和背景色的计算机函数
//}
//
//void gotoxy(int x, int y)    //坐标跳转
//{
...
//	//SetConsoleCursorPosition是一个API（应用程序编程接口），可以设置设置控制台窗口中光标的位置。
//}